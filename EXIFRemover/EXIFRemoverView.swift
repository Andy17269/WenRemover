...
struct EXIFRemoverView: View {
    ...
    private func copySingleImageToClipboard() {
        guard let url = imageURLs.first else { return }
        isProcessing = true
        statusMessage = localizedString("status.processing")
        ...
        Task.detached {
            do {
                ...
                let outputURL = try ImageStripper.stripMetadata(
                    inputURL: url,
                ...
                if let image = NSImage(contentsOf: outputURL) {
                    await MainActor.run {
                        ...
                        if let urlRef = outputURL as? NSURL {
...
struct OutputSettingsView: View {
    ...
    @AppStorage("languagePreference") private var languagePreference: String = "system"
    var blurIntensity: Binding<BlurIntensity>?

    init(configuration: OutputConfiguration, blurIntensity: Binding<BlurIntensity>? = nil, onSave: @escaping (OutputConfiguration) -> Void) {
        ...
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                if let blurIntensity = blurIntensity {
                    VStack(alignment: .leading, spacing: 8) {
                        Text(localizedString("privacy.intensity.label"))
                            .font(.headline)
                        Picker("", selection: blurIntensity) {
                            ForEach(BlurIntensity.allCases) { intensity in
                                Text(localizedString("privacy.intensity.\(intensity.rawValue.lowercased())")).tag(intensity)
                            }
                        }
